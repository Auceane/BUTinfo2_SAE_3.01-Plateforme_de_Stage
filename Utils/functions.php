...
//Fonction spéciale sans base de données

function userExist($user){
	return isset($user,$_SESSION['users']);
}
	...
	"eleve1"=>
		[
			...
			"attribut"=>[
				"nomPersonne"=>"Élève",
				"prenomPersonne"=>"1",
				"personne"=>"Élève numéro 1",
				"role"=>"Étudiant",
				"mail"=>"lorem@g.com",
				"Formation"=>"BUT"
			],
			"last50"=>[
				["type"=>"CV","date"=>"2023-01-13 15:25:58","url"=>"cv-exemple.pdf"],
				["type"=>"Lettre_de_Motivation","date"=>"2022-12-02 12:13:44","url"=>"lm-exemple.pdf"]
			],
	...
	"eleve3"=>
		[
			...
			"attribut"=>[
				"nomPersonne"=>"Élève",
				"prenomPersonne"=>"3",
				"personne"=>"Élève numéro 3",
				"role"=>"Étudiant",
				"mail"=>"lorem2@g.com",
				"Formation"=>"Licence"
			],
			"last50"=>[
				["type"=>"Lettre_de_Motivation","date"=>"2023-02-02 12:13:44","url"=>"lm-exemple.pdf"]
			],
		],
	"professeur1"=>
		[
			"id"=>4,
			"attribut"=>[
				"nomPersonne"=>"Professeur",
				"prenomPersonne"=>"1",
				"personne"=>"Professeur numéro 1",
				"role"=>"Enseignant Tuteur",
				"mail"=>"ipsum2@g.com",
				"Formation"=>"BUT"
			],
		],
	"professeur2"=>
		[
			...
			"attribut"=>[
				"nomPersonne"=>"Professeur",
				"prenomPersonne"=>"2",
				"personne"=>"Professeur numéro 2",
				"role"=>"Enseignant Validateur",
				"mail"=>"lorem3@g.com",
				"Formation"=>"Licence"
			],
		],
...
function derniersDoc(){
	$infos=$GLOBALS['info'];
	$docs=[];
	$count=0;
	for($infos as $info){
		if($info["attribut"]['role']=="Étudiant"){
			$student=$info["attribut"]['personne'];
			$id=$info['id'];
			if(isset($info["last50"]) && $info["last50"]!=[]){
				for($info["last50"] as $doc){
					$d=[];
					$d['type']=$doc["type"];
					$d['docID']=$count;
					$d['personne']=$student;
